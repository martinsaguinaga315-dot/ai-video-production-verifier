...
$scan = & $buildPython -c "from pathlib import Path; from packaging.release_utils import scan_tree,scan_zip; import sys; paths=[Path(r'$distDir'),Path(r'$portable')]; findings=scan_tree(paths[0])+scan_zip(paths[1]); print('\\n'.join(findings)); sys.exit(bool(findings))"
