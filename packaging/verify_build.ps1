...
if(-not(Test-Path -LiteralPath $exe)){throw "Missing frozen executable: $exe"}; foreach($required in @('examples','LICENSE','_internal')){if(-not(Test-Path -LiteralPath (Join-Path $DistRoot "$AppName\$required"))){throw "Missing bundled resource: $required"}}
